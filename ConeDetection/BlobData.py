...
def get_BLOBS_and_cone_array(folder,filename):
    ...
    for blob in contours:
        ...
        cv2.rectangle(img, (bbox[0], bbox[1]), 
                                    (int(bbox[0]+bbox[2]), int(bbox[1]+bbox[3])), (255, 0, 255), 2)

    #img_2 = cv2.resize(img, (0,0), fx=4, fy=4)
...
for filename in os.listdir(folder):
    ...
    i=0  
    sorted_cont_props = sort_blobs_area(cont_props,min_area=100)
    labeled_features = []

    stop = False

    #gets the RGB image from input folder
    img_BGR = cv2.imread(os.path.join(input_path, filenames_in_input_folder[file_num]), cv2.IMREAD_UNCHANGED)
    ...
    for cont in sorted_cont_props:
        ...
        cropped_img_BGR = img_BGR[y1:y1+h, x1:x1+w]
            
        is_cone = cone_or_not(cropped_img_BLOB,cropped_img_BGR)
        ...
        data.append([[sorted_cont_props[i]], [is_cone]])
        i+=1
